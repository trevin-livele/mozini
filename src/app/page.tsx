        ...
        {heroSlides.map((slide, i) => (
          <div key={i} className={`absolute inset-0 flex items-center transition-opacity duration-700 ${i === heroIndex ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <div className="hero-content relative z-10 px-5 md:pl-12 lg:pl-20 max-w-[90%] md:max-w-[500px]">
              ...
              <Link href="/shop" className="inline-block bg-[var(--copper)] text-white px-6 md:px-8 py-2.5 md:py-3 rounded text-xs md:text-sm font-medium uppercase tracking-wider hover:bg-[var(--copper-dark)] hover:-translate-y-0.5 hover:shadow-lg transition-all">
                Shop Now
              </Link>
            </div>
          ...
          <div className="flex justify-center gap-4 md:gap-6 mb-7 md:mb-9 overflow-x-auto pb-2">
            {['featured', 'new', 'best'].map((tab) => (
              <button
                ...
                className={`text-xs md:text-sm font-medium uppercase tracking-wider pb-1.5 border-b-2 transition-all whitespace-nowrap ${activeTab === tab ? 'text-[var(--copper)] border-[var(--copper)]' : 'text-[var(--text-light)] border-transparent hover:text-[var(--copper)]'}`}
              >
