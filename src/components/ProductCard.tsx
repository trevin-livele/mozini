...
export default function ProductCard({ product }: ProductCardProps) {
  const { addToCart, toggleWishlist, isInWishlist, loading } = useStore();
  const { user } = useAuth();
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!user) { router.push('/login?redirectTo=/shop'); return; }
    if (loading) return;
      ...
      <div ref={cardRef} className="bg-white rounded-lg overflow-hidden border border-[var(--border)] relative cursor-pointer transition-all hover:shadow-lg hover:border-transparent hover:-translate-y-1 group">
        ...
        <div className="relative h-[220px] bg-[var(--bg-soft)] flex items-center justify-center overflow-hidden">
          ...
          <div className="absolute bottom-[-50px] left-0 right-0 flex justify-center gap-2 p-2.5 transition-all duration-300 group-hover:bottom-2.5">
            <button onClick={handleAddToCart} className="min-w-[44px] min-h-[44px] w-9 h-9 rounded-full bg-white flex items-center justify-center shadow-sm hover:bg-[var(--copper)] hover:text-white hover:scale-110 transition-all" title="Add to Cart">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
            </button>
            <button onClick={handleToggleWishlist} className={`min-w-[44px] min-h-[44px] w-9 h-9 rounded-full flex items-center justify-center shadow-sm hover:scale-110 transition-all ${inWishlist ? 'bg-[var(--red)] text-white' : 'bg-white hover:bg-[var(--copper)] hover:text-white'}`} title="Wishlist">
            ...
            <button onClick={handleWhatsAppInquiry} className="min-w-[44px] min-h-[44px] w-9 h-9 rounded-full bg-white flex items-center justify-center shadow-sm hover:bg-[#25D366] hover:text-white hover:scale-110 transition-all" title="WhatsApp Inquiry">
