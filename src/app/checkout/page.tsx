...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await createOrder(serverCart, {
      ...
      // Sync the Zustand store (cart is now empty server-side)
      await syncCart();
  ...
  if (loadingCart) {
    return <div className="text-center py-20 text-[var(--text-light)]">Loading checkout...</div>;
  }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-8 md:gap-12 items-start">
              <div>
                ...
                <div className="mb-4 md:mb-5">
                  ...
                  <select name="county" required className="w-full px-3 md:px-4 py-2.5 md:py-3 border border-[var(--border)] rounded text-sm focus:border-[var(--copper)] transition-colors">
                ...
                <div className="mb-4 md:mb-5">
                  ...
                  <input name="address" required placeholder="Street address, apartment, etc." className="w-full px-3 md:px-4 py-2.5 md:py-3 border border-[var(--border)] rounded text-sm focus:border-[var(--copper)] transition-colors" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-5">
                  <div>
                  ...
                  <div>
                    ...
                    <input name="postalCode" placeholder="00100" className="w-full px-3 md:px-4 py-2.5 md:py-3 border border-[var(--border)] rounded text-sm focus:border-[var(--copper)] transition-colors" />
                  </div>
                </div>
                <div className="mb-6 md:mb-8">
                  <label className="block text-xs md:text-sm font-medium text-[var(--dark)] mb-1.5">Order Notes</label>
                  <textarea name="notes" placeholder="Notes about your order..." className="w-full px-3 md:px-4 py-2.5 md:py-3 border border-[var(--border)] rounded text-sm focus:border-[var(--copper)] transition-colors min-h-[80px] md:min-h-[100px] resize-y"></textarea>
                </div>
              ...
              <div className="bg-[var(--bg-soft)] p-6 md:p-8 rounded-xl lg:sticky lg:top-24">
                ...
                <button type="submit" disabled={submitting} className="hidden lg:block w-full bg-[var(--copper)] text-white py-3 md:py-3.5 rounded text-xs md:text-sm font-medium uppercase tracking-wider hover:bg-[var(--copper-dark)] transition-colors mt-4 md:mt-5 disabled:opacity-50">
                  {submitting ? 'Processing...' : `Place Order — ${formatPrice(total)}`}
                </button>
              </div>
            </div>
